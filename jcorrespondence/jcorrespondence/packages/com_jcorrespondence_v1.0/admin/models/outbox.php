 ...
 * @since	3.6.0
 */
class JcorrespondenceModelOutbox extends JModelList
{
        ...
        protected function getListQuery($postId=0)
	{
		...
		$query->join('INNER',$db->quoteName('#__correspondence_users').' AS b on b.postid=a.postid');
                ...
                $query->where('b.box=\'1\''); // 1->inbox, 0->outbox
	...
	protected function _getListQuery($postId=0)
	{
        ...
        public function get_myItems($postId=0)
	{
		// Load the list items.
		$query = $this->_get_myListQuery($postId);
		$items = $this->_getList($query, $this->getStart(), $this->getState('list.limit'));
        ...
        protected function _get_myListQuery($postId=0)
	{
		$this->query = $this->get_myListQuery($postId);
		return $this->query;
	}
       
        protected function get_myListQuery($postId=0)
	{
		// Create a new query object.
		$db	= $this->getDbo();
		$query	= $db->getQuery(true);
		...
		$query->join('INNER',$db->quoteName('#__correspondence_users').' AS b on b.postid=a.postid');
                ...
                $query -> where('a.postid='.(int)$postId);
		...
		$user	= JFactory::getUser();
                ...
                $query->where('head.postid IN (SELECT users.postid
					...
					WHERE users.userid='.(int)$userId.' AND users.box=\'1\')');
